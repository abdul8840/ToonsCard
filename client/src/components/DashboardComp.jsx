...
export default function DashboardComp() {
  ...
  useEffect(() => {
    ...
    const fetchCategory = async () => {
      try {
        const res = await fetch('/api/category/getcategory?limit=5');
        const data = await res.json();
        if (res.ok) {
    ...
    <div className='p-3 md:mx-auto'>
      <div className='flex-wrap flex gap-4 justify-center'>
        ...
        <div className='flex flex-col p-3 dark:bg-slate-800 gap-4 md:w-72 w-full rounded-md shadow-md'>
          <div className='flex justify-between'>
            <div className=''>
              <h3 className='text-gray-500 text-md uppercase'>Total Category</h3>
      ...
      <div className='flex flex-wrap gap-4 py-3 mx-auto justify-center'>
        ...
        <div className='flex flex-col w-full md:w-auto shadow-md p-2 rounded-md dark:bg-gray-800'>
          <div className='flex justify-between  p-3 text-sm font-semibold'>
            ...
            <Button outline gradientDuoTone='purpleToPink'>
              <Link to={'/dashboard?tab=category'}>See all</Link>
            </Button>
          </div>
          <Table hoverable>
            <Table.Head>
              ...
              categories.map((post) => (
                <Table.Body key={post._id} className='divide-y'>
                  <Table.Row className='bg-white dark:border-gray-700 dark:bg-gray-800'>
                    ...
                    <Table.Cell className='w-64'>{post.name}</Table.Cell>
                  </Table.Row>
                </Table.Body>
              ))}
          </Table>
        ...
        <div className='flex flex-col w-full md:w-auto shadow-md p-2 rounded-md dark:bg-gray-800'>
          ...
          <Table hoverable>
            <Table.Head>
              <Table.HeadCell>Card Image</Table.HeadCell>
